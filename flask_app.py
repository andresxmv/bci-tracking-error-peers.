...
def category_levels(name: str):
    ...
    if payload is None:
        # embedded series keys follow the original xlsx stem; fallback through metrics metadata
    ...
    if not payload:
        return pd.DataFrame()
    return pd.DataFrame(payload["valores"], index=pd.to_datetime(payload["fechas"])).apply(pd.to_numeric, errors="coerce").sort_index()
...
def configured_columns(name: str, levels: pd.DataFrame):
    ...
    peers = [c for c in peers if c is not None]
    return bci, peers
...
def ewma_te(active: pd.Series, lam=.94, annualize=True):
    ...
    return math.sqrt(var * (52 if annualize else 1))
...
def historical_te(name: str):
    ...
    returns = levels[[bci_col, *peer_cols]].pct_change(fill_method=None).dropna(how="any")
    if len(returns) < 8:
        return {"labels": [], "bci": [], "p75": []}
    funds = [bci_col, *peer_cols]
    out = pd.DataFrame(index=returns.index, columns=funds, dtype=float)
    _, cfg = config_by_name(name)
    ...
    for i in range(7, len(returns)):
        ...
        for fund in funds:
            ...
            out.loc[returns.index[i], fund] = ewma_te(window[fund] - window[others].mean(axis=1), annualize=annualize)
...
def peer_rows_for(name: str):
    _, cfg = config_by_name(name)
    runs = [str(cfg["bci"]), *[str(x) for x in cfg.get("peers", [])]] if cfg else []
...
def fund_dashboard(selected_run: str):
    ...
    # Panel Python define percentil como fracción de peers que superan al BCI:
    # 0.00 = mejor; cuartil = ceil(percentil*4), con mínimo 1.
...
@app.get("/health")
def health():
    return {"ok": True, "funds": len(CONFIG), "reference_rows": len(REFERENCE)}, 200
